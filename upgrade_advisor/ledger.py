...
def _episodes_with_pairs(entries: List[dict]) -> List[dict]:
    """Latest entry per target that carries both floor and reference."""
    by_target = {}
    for e in entries:
        if e.get("floor") is not None and e.get("reference") is not None:
...
def project_reference(entries: List[dict], new_floor: float
    ...
    if est is None:
        return None
    beta, old, new = est
    proj = new["reference"] + beta * (new_floor - new["floor"])
